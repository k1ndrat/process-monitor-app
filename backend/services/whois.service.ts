...
import { type TWhoisData } from "../../types/index.ts";
    ...
    exec(command, { maxBuffer: 1024 * 1024 * 5 }, (error, stdout) => {
      ...
      const parsedData: TWhoisData = {
        ...
        expirationDate: extract(
          /Registry Expiry Date:\s*(.+)|Registrar Registration Expiration Date:\s*(.+)/i,
        ),
        ...
        nameServers: [
          ...new Set(
            [...rawText.matchAll(/Name Server:\s*(.+)/gi)].map((m) =>
        ...
        statuses: [
          ...new Set(
            [...rawText.matchAll(/Domain Status:\s*([^\s]+)/gi)].map((m) =>
              m[1]!.trim(),
            ),
          ),
        ],
