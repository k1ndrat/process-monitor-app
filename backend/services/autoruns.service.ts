...
export const getAutorunsData = (): Promise<TAutorun[]> => {
  return new Promise((resolve, reject) => {
    const cmd = `"C:\\Users\\haker\\Downloads\\Autoruns\\autorunsc64.exe" -c -h -v -s -m -accepteula -nobanner`;

    exec(
      cmd,
      { maxBuffer: 1024 * 1024 * 10, encoding: "buffer" },
      async (error: Error | null, stdoutBuffer: Buffer) => {
        if (error) {
        ...
        const rawAutoruns = lines
          .map((line: string) => {
            ...
            if (entryName === "Entry" && imagePath === "Image Path")
              return null;
            ...
            if (parts[12] && parts[12].includes("|")) {
              ...
              vtDetections = parseInt(vtParts[0] ?? "0", 10);
              vtTotal = parseInt(vtParts[1] ?? "0", 10);
            }
          ...
          .filter((item): item is TAutorun => item !== null);
