...
app.get("/api/tasks", (req: Request, res: Response) => {
  exec("tasklist /fo csv /nh", (error, stdout) => {
    ...
    const heavyTasks = sortedTasks.filter(t => t.memUsage >= MEMORY_THRESHOLD_KB);

    const lightTasks = sortedTasks.filter(t => t.memUsage < MEMORY_THRESHOLD_KB);
    
    if (lightTasks.length > 0) {
        const othersMemUsage = lightTasks.reduce((acc, curr) => acc + curr.memUsage, 0);
...
app.listen(3001, () => console.log("Server runs on http://localhost:3001"));